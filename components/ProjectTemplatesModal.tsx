        ...
        <div className="fixed inset-0 bg-modal-overlay flex justify-center items-center z-50">
            <div className="bg-secondary border border-primary rounded-lg p-6 w-full max-w-4xl max-h-[90vh] flex flex-col">
                <div className="flex justify-between items-center mb-4 flex-shrink-0">
                    <h2 className="text-2xl font-bold">{t('projectTemplates')}</h2>
                    ...
                    <div className="col-span-2 overflow-y-auto pr-2">
                      {(selectedTemplate || isCreatingNew) && (
                        <div className="space-y-4">
                            <h3 className="text-xl font-semibold text-primary">{isCreatingNew ? t('newTemplate') : t('editTemplate')}</h3>
                             <div>
                                <label className="block text-sm font-medium text-secondary mb-1">{t('templateName')}</label>
                             ...
                             <div>
                                <label className="block text-sm font-medium text-secondary mb-1">{t('descriptionLabel')}</label>
                             ...
                             <h4 className="text-lg font-semibold text-primary pt-2 border-t border-primary">{t('templateEvents')}</h4>
                                 ...
                                 {formData.events.map((event, index) => (
                                     <div key={index} className="bg-tertiary p-3 rounded-md space-y-2 relative">
                                        ...
                                        <div className="grid grid-cols-2 gap-2">
                                            <div>
                                                <label className="text-xs text-secondary">{t('eventName')}</label>
                                             ...
                                             <div>
                                                <label className="text-xs text-secondary">{t('daysFromStart')}</label>
                             ...
                             <div className="flex justify-end items-center pt-4 space-x-2">
                                ...
                                <button onClick={handleSave} className="px-5 py-2 rounded-md bg-wha-blue text-white font-bold hover:bg-blue-600">{t('saveTemplate')}</button>
                            </div>
                        </div>
                      )}
                    </div>
