    ...
    <div className="fixed inset-0 bg-modal-overlay flex justify-center items-center z-50" role="dialog" aria-modal="true" aria-labelledby="add-contact-title">
      <form onSubmit={handleSubmit} className="bg-secondary border border-primary rounded-lg p-8 w-full max-w-lg">
        <div className="flex justify-between items-center mb-6">
            <h2 id="add-contact-title" className="text-2xl font-bold">{contactToEdit ? t('editContactTitle') : t('addContactTitle')}</h2>
        ...
        <div className="space-y-4">
            <div className="grid grid-cols-2 gap-4">
                <div>
                    <label htmlFor="name" className="block text-sm font-medium text-secondary mb-1">{t('fullName')}</label>
                ...
                <div>
                    <label htmlFor="role" className="block text-sm font-medium text-secondary mb-1">{t('roleTitle')}</label>
            ...
            <div>
                <label htmlFor="locationId" className="block text-sm font-medium text-secondary mb-1">{t('associatedLocation')}</label>
             ...
             <div className="grid grid-cols-2 gap-4">
                <div>
                    <label htmlFor="phone" className="block text-sm font-medium text-secondary mb-1">{t('phone')}</label>
                ...
                <div>
                    <label htmlFor="email" className="block text-sm font-medium text-secondary mb-1">{t('email')}</label>
             ...
             <div>
                <label htmlFor="messenger" className="block text-sm font-medium text-secondary mb-1">{t('messenger')}</label>
                <input type="text" id="messenger" value={messenger} onChange={(e) => setMessenger(e.target.value)} className="w-full px-3 py-2 bg-input border border-primary rounded-lg" placeholder={t('messengerPlaceholder')} />
            </div>
        </div>
        ...
        <div className="flex justify-end space-x-4 pt-6">
          ...
          <button type="submit" className="px-5 py-2 rounded-md bg-wha-blue text-white font-bold">{contactToEdit ? t('saveChanges') : t('saveContact')}</button>
        </div>
      </form>
    </div>
  );
