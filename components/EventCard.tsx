...
export const EventCard: React.FC<EventCardProps> = ({ event, onLocationClick, onWhenClick }) => {
  return (
    <div className="bg-slate-800/50 border border-slate-700 rounded-lg p-5 transition-all duration-300 hover:border-wha-blue hover:shadow-lg hover:shadow-wha-blue/10">
      <div className="flex flex-col md:flex-row justify-between md:items-center mb-4">
        <h3 className="text-xl font-bold text-text-light">{event.what.name}</h3>
      </div>
      {event.what.description && <p className="text-slate-400 mb-6">{event.what.description}</p>}
      
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        <div className="space-y-3">
            {event.who.length > 0 && <div className="flex items-center space-x-2">
                <div className="w-6 text-slate-400"><PersonIcon/></div>
                <div className="flex flex-wrap gap-2">
                    {event.who.map(p => <EntityTag key={p.id} label={p.name} type={EntityType.Who} />)}
                </div>
            </div>}
             <div className="flex items-start space-x-2">
                <div className="w-6 text-slate-400 pt-1"><PinIcon/></div>
                <div className="flex flex-wrap gap-2">
                    <EntityTag 
        ...
        <div className="flex items-start space-x-2 md:pl-6 md:border-l md:border-slate-700">
             <div className="w-6 text-slate-400 pt-1"><CalendarIcon/></div>
             <div className="flex flex-wrap gap-2">
                <EntityTag 
