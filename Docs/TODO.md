...
-   [ ] **Location Intelligence & Management (New Epic):**
    -   [ ] **Phase 1: First-Class Locations:**
        -   [ ] Rework data model to have a dedicated `locations` state with rich fields (alias, phone, website, portalUrl, notes).
        -   [ ] Update `EventNode` to use `locationId` instead of embedding location data.
        -   [ ] Convert "Where" input in `AddEventForm` to a searchable dropdown for existing locations, with a "+ New Location" button.
